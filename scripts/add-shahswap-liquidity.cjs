...
async function main() {
    console.log("💧 Adding Liquidity to ShahSwap Pairs...\n");
    ...
    try {
        ...
        console.log("🔍 Checking current token balances...");
        ...
        // Calculate liquidity amounts based on available balances (using 80% of available balance)
        const LIQUIDITY_AMOUNTS = {
            "SHAH-USDT": {
                shahAmount: shahBalance * 80n / 1000n, // Use 8% of SHAH balance
                usdtAmount: usdtBalance * 80n / 100n, // Use 80% of USDT balance
                description: `${ethers.formatEther(shahBalance * 80n / 1000n)} SHAH + ${ethers.formatUnits(usdtBalance * 80n / 100n, 6)} USDT`
            },
            "SHAH-DAI": {
                shahAmount: shahBalance * 80n / 1000n, // Use 8% of SHAH balance
                daiAmount: daiBalance * 80n / 100n, // Use 80% of DAI balance
                description: `${ethers.formatEther(shahBalance * 80n / 1000n)} SHAH + ${ethers.formatEther(daiBalance * 80n / 100n)} DAI`
            }
        };

        // Only add pairs where we have sufficient balance
        const pairsToAdd = [];
        
        if (shahBalance > ethers.parseEther("1") && usdtBalance > ethers.parseUnits("1", 6)) {
            pairsToAdd.push("SHAH-USDT");
        }
        
        if (shahBalance > ethers.parseEther("1") && daiBalance > ethers.parseEther("1")) {
            pairsToAdd.push("SHAH-DAI");
        }

        if (pairsToAdd.length === 0) {
            console.log("❌ Insufficient balance to add liquidity to any pairs");
            console.log("💡 Please add more tokens to your wallet and try again");
            return;
        }

        console.log(`✅ Will add liquidity to ${pairsToAdd.length} pairs: ${pairsToAdd.join(", ")}\n`);
        ...
        for (const pairName of pairsToAdd) {
            console.log(`💧 Adding ${pairName} liquidity...\n`);
            ...
            try {
                ...
                console.log(`   ✅ ${pairName} liquidity added: ${addLiquidityTx.hash}`);
                ...
                results.pairs[pairName] = {
                    ...
                    tokenAmount: pairName === "SHAH-USDT" ? amounts.usdtAmount.toString() : amounts.daiAmount.toString(),
                    status: "SUCCESS"
                };
                
            } catch (error) {
                console.log(`   ❌ ${pairName} liquidity failed: ${error.message}`);
                results.pairs[pairName] = { status: "FAILED", error: error.message };
            }
            
            console.log("");
        }

        // Register successful pairs in Oracle
        ...
        // Final Summary
        console.log("🎉 ShahSwap Liquidity Addition Complete!\n");
        ...
        console.log("   4. Test remove liquidity when ready");

        // Save results to file
        const resultsPath = path.join(__dirname, "..", "shahswap-liquidity-results.json");
