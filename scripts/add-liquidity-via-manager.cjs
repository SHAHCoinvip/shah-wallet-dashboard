...
async function main() {
    console.log("💧 Adding Liquidity via ShahSwapLiquidityManager...\n");
    ...
    try {
        ...
        // Step 1: Add liquidity via ShahSwapLiquidityManager
        console.log("💧 Step 1: Adding liquidity via ShahSwapLiquidityManager...\n");

        try {
            console.log(`   Adding liquidity: ${LIQUIDITY_AMOUNTS.description}`);
            
            // Check current allowances for liquidity manager
            const shahAllowance = await shahToken.allowance(deployer.address, LIQUIDITY_MANAGER);
            const usdtAllowance = await usdtToken.allowance(deployer.address, LIQUIDITY_MANAGER);
            ...
            if (shahAllowance < LIQUIDITY_AMOUNTS.shahAmount) {
                console.log(`   Approving SHAH tokens for liquidity manager...`);
                const approveShahTx = await shahToken.approve(LIQUIDITY_MANAGER, LIQUIDITY_AMOUNTS.shahAmount);
                await approveShahTx.wait();
                console.log(`   ✅ SHAH tokens approved: ${approveShahTx.hash}`);
            }
            
            if (usdtAllowance < LIQUIDITY_AMOUNTS.usdtAmount) {
                console.log(`   Approving USDT tokens for liquidity manager...`);
                const approveUsdtTx = await usdtToken.approve(LIQUIDITY_MANAGER, LIQUIDITY_AMOUNTS.usdtAmount);
                await approveUsdtTx.wait();
                console.log(`   ✅ USDT tokens approved: ${approveUsdtTx.hash}`);
            }
            
            // Add liquidity using the liquidity manager
            console.log(`   Adding liquidity via liquidity manager...`);
            const addLiquidityTx = await liquidityManager.addLiquidity(
                TOKENS.USDT,           // token address
                LIQUIDITY_AMOUNTS.shahAmount,  // SHAH amount
                LIQUIDITY_AMOUNTS.usdtAmount,  // USDT amount
                0                      // minimum LP tokens (0 for now)
            );
        ...
        console.log("   • Liquidity added via ShahSwapLiquidityManager");
        ...
        console.log(`   Liquidity Manager: https://etherscan.io/address/${LIQUIDITY_MANAGER}`);
