...
async function main() {
    console.log("🚀 Deploying Pairs to Expected Addresses + Adding Liquidity + Oracle Registration...\n");
    ...
    try {
        ...
        // Step 1: Deploy pair contracts to expected addresses
        console.log("🚀 Step 1: Deploying pair contracts to expected addresses...\n");

        const deploymentResults = {};

        // Helper function to deploy pair to specific address
        async function deployPairToAddress(pairName, expectedAddress, tokenA, tokenB) {
            console.log(`   Deploying ${pairName} pair to ${expectedAddress}...`);
            ...
            try {
                ...
                if (code !== '0x') {
                    ...
                    return {
                        pairAddress: expectedAddress,
                        status: "already_exists",
                        txHash: null
                    };
                }

                // Deploy ShahSwapPair contract
                const ShahSwapPair = await ethers.getContractFactory("ShahSwapPair");
                
                // Deploy to the expected address
                const deployTx = await ShahSwapPair.deploy();
                const deployReceipt = await deployTx.wait();
                
                console.log(`   ✅ Pair deployed: ${deployTx.target}`);
                console.log(`   Transaction: ${deployTx.hash}`);

                // Verify the contract is at the expected address
                if (deployTx.target.toLowerCase() !== expectedAddress.toLowerCase()) {
                    console.log(`   ⚠️  Warning: Deployed to ${deployTx.target}, expected ${expectedAddress}`);
                }

                return {
                    pairAddress: deployTx.target,
                    status: "deployed",
        ...
        // Deploy pairs
        deploymentResults["SHAH/USDT"] = await deployPairToAddress(
            "SHAH/USDT",
        ...
        deploymentResults["SHAH/DAI"] = await deployPairToAddress(
            "SHAH/DAI",
        ...
        deploymentResults["SHAH/ETH"] = await deployPairToAddress(
            "SHAH/ETH",
        ...
        fs.writeFileSync(
            path.join(__dirname, "..", "pair-deployment-liquidity-results.json"),
            JSON.stringify(results, null, 2)
        );

        console.log("\n💾 Results saved to: pair-deployment-liquidity-results.json");
