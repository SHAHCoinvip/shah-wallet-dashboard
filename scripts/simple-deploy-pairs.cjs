...
async function main() {
    console.log("🚀 Simple Pair Deployment + Liquidity + Oracle Registration...\n");
    ...
    try {
        ...
        // Step 1: Deploy pair contracts
        console.log("🚀 Step 1: Deploying pair contracts...\n");

        const deploymentResults = {};

        // Helper function to deploy pair
        async function deployPair(pairName, tokenA, tokenB) {
            console.log(`   Deploying ${pairName} pair...`);
            ...
            try {
                ...
                console.log(`   Transaction: ${pair.deploymentTransaction().hash}`);

                return {
                    pairAddress,
                    status: "deployed",
                    txHash: pair.deploymentTransaction().hash
                };
        ...
        // Deploy pairs
        deploymentResults["SHAH/USDT"] = await deployPair(
            "SHAH/USDT",
            TOKENS.SHAH,
            TOKENS.USDT
        );
        ...
        deploymentResults["SHAH/DAI"] = await deployPair(
            "SHAH/DAI",
            TOKENS.SHAH,
            TOKENS.DAI
        );
        ...
        deploymentResults["SHAH/ETH"] = await deployPair(
            "SHAH/ETH",
            TOKENS.SHAH,
            TOKENS.WETH
        );
        ...
        fs.writeFileSync(
            path.join(__dirname, "..", "simple-deployment-results.json"),
            JSON.stringify(results, null, 2)
        );

        console.log("\n💾 Results saved to: simple-deployment-results.json");
