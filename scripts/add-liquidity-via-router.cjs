...
async function main() {
    console.log("💧 Adding Liquidity via ShahSwapRouterV2...\n");
    ...
    try {
        ...
        // Step 1: Add liquidity via ShahSwapRouterV2
        console.log("💧 Step 1: Adding liquidity via ShahSwapRouterV2...\n");

        try {
            console.log(`   Adding liquidity: ${LIQUIDITY_AMOUNTS.description}`);
            
            // Check current allowances for router
            const shahAllowance = await shahToken.allowance(deployer.address, ROUTER);
            const usdtAllowance = await usdtToken.allowance(deployer.address, ROUTER);
            ...
            if (shahAllowance < LIQUIDITY_AMOUNTS.shahAmount) {
                ...
                const approveShahTx = await shahToken.approve(ROUTER, LIQUIDITY_AMOUNTS.shahAmount);
            ...
            if (usdtAllowance < LIQUIDITY_AMOUNTS.usdtAmount) {
                ...
                const approveUsdtTx = await usdtToken.approve(ROUTER, LIQUIDITY_AMOUNTS.usdtAmount);
            ...
            console.log(`   Adding liquidity via router...`);
        ...
        console.log("   • Liquidity added via ShahSwapRouterV2");
        ...
        console.log(`   Router: https://etherscan.io/address/${ROUTER}`);
