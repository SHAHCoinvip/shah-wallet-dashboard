...
async function main() {
    console.log("🏭 Creating Pairs + Adding Liquidity + Oracle Registration...\n");
    ...
    try {
        ...
        // Step 1: Create pairs using factory
        console.log("🏭 Step 1: Creating pairs using factory...\n");
        ...
        // Create pairs
        pairResults["SHAH/USDT"] = await createPairAndAddLiquidity(
            "SHAH/USDT",
            TOKENS.SHAH,
            TOKENS.USDT,
            LIQUIDITY_AMOUNTS["SHAH/USDT"].shahAmount,
            LIQUIDITY_AMOUNTS["SHAH/USDT"].usdtAmount
        );
        console.log("");

        pairResults["SHAH/DAI"] = await createPairAndAddLiquidity(
            "SHAH/DAI",
            TOKENS.SHAH,
            TOKENS.DAI,
            LIQUIDITY_AMOUNTS["SHAH/DAI"].shahAmount,
            LIQUIDITY_AMOUNTS["SHAH/DAI"].daiAmount
        );
        console.log("");

        pairResults["SHAH/ETH"] = await createPairAndAddLiquidity(
            "SHAH/ETH",
            TOKENS.SHAH,
            TOKENS.WETH,
            LIQUIDITY_AMOUNTS["SHAH/ETH"].shahAmount,
            LIQUIDITY_AMOUNTS["SHAH/ETH"].ethAmount
        );
        console.log("");
        ...
        for (const [pairName, result] of Object.entries(pairResults)) {
            if (result.status === "created" || result.status === "already_exists") {
                ...
                liquidityResults[pairName] = await addLiquidityToPair(
                    pairName,
                    result.pairAddress,
                    liquidity.shahAmount === liquidity.shahAmount ? TOKENS.SHAH : (pairName.includes('USDT') ? TOKENS.USDT : pairName.includes('DAI') ? TOKENS.DAI : TOKENS.WETH),
        ...
        // Summary
        console.log("🎉 Pair Creation + Liquidity Addition + Oracle Registration Complete!\n");
        ...
        fs.writeFileSync(
            path.join(__dirname, "..", "pair-creation-liquidity-results.json"),
            JSON.stringify(results, null, 2)
        );

        console.log("\n💾 Results saved to: pair-creation-liquidity-results.json");
